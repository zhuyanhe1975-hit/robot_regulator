...
def irb2400_pdff_env_cfg(*, play: bool = False) -> ManagerBasedRlEnvCfg:
    """Fixed-gain PD + learned residual FF (no inverse dynamics).

    Physics 2ms, control/env step 20ms (decimation=10). Used to isolate whether
    the inverse-dynamics term is causing instability.
    """
    ...
    actions: dict[str, ActionTermCfg] = {
        "pd_ff": joint_pd_fixed_idff_action.JointPdFixedIdFfActionCfg(
            asset_name="robot",
            actuator_names=(".*",),
...
def irb2400_pdff_coarse_dt_env_cfg(*, play: bool = False) -> ManagerBasedRlEnvCfg:
    """Same as PDFF, but with coarse physics timestep to quantify numerical effects.

    Physics 20ms, control/env step 20ms (decimation=1).
    """
    cfg = irb2400_pdff_env_cfg(play=play)
    cfg.decimation = 1
    cfg.sim.mujoco.timestep = 0.02
    # Coarse physics timestep requires much lower gains for stability.
    # (Rule of thumb: dt x10 => kp /~100, kd /~10)
    if cfg.actions is not None and "pd_ff" in cfg.actions:
        act = cfg.actions["pd_ff"]
        if hasattr(act, "kp"):
            act.kp = 10.0
        if hasattr(act, "kd"):
            act.kd = 8.0
