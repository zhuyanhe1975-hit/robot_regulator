...
from mjlab_irb2400.mdp import (
    joint_pd_gain_ff_action,
    joint_sine_command,
    joint_tracking_rewards,
)

# Joint friction (object model): Coulomb + viscous.
# MuJoCo's built-in "frictionloss" is Coulomb-like; "damping" is viscous.
...
def irb2400_joint_gain_ff_env_cfg(*, play: bool = False) -> ManagerBasedRlEnvCfg:
    ...
    actions: dict[str, ActionTermCfg] = {
        "pd_gains_ff": joint_pd_gain_ff_action.JointPdGainFfActionCfg(
            asset_name="robot",
            actuator_names=(".*",),
            command_name="joint_pos",
            # Baseline (curriculum start) gains; NN gradually modulates around these.
            kp_scale=800.0,
            kp_offset=928.0,
            kd_scale=120.0,
            kd_offset=80.0,
            ...
            clip_kd=(10.0, 400.0),
            # NN residual: friction compensation parameters updated at 100Hz; PD+ID run at 1kHz.
            # Keep residual authority comparable to the plant friction to avoid over-compensation.
            ...
            # Disable curriculum during play/eval so full residual range is used.
            # With many envs, timesteps accumulate fast; delay/slow curriculum substantially.
            friction_warmup_steps=0 if play else 200_000_000,
            friction_ramp_steps=0 if play else 300_000_000,
            gain_warmup_steps=0 if play else 50_000_000,
            gain_ramp_steps=0 if play else 100_000_000,
            kp_slew_rate=20_000.0,
            kd_slew_rate=2_000.0,
    ...
    rewards = {
        ...
        # Smooth scheduling at the *applied* update rate (controller holds at 100Hz).
        "action_rate_l2": RewardTermCfg(
            func=joint_tracking_rewards.held_action_rate_l2,
            weight=-1e-2,
            params={"term_name": "pd_gains_ff"},
        ),
    }
